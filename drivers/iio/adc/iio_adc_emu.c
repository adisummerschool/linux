...
#define EMU_ADDR_MSK		GENMASK(14,8) //specify the interval and sets to 1 all
				      //bits inside the interval
#define EMU_DATA_MSK		GENMASK(7,0)
#define EMU_PWR_REG		0x02
#define EMU_PWR_ENABLE		0x0
#define EMU_PWR_DISABLE		0x20
...
static int iio_adc_emu_spi_write(struct iio_adc_emu_st *st, u8 addr, u8 data) {
	...
	tx = FIELD_PREP(EMU_RDWR_MSK, 0) | FIELD_PREP(EMU_ADDR_MSK, addr)
...
static int iio_adc_emu_read_chan(struct iio_adc_emu_st *st, int channel,
				 u16 *readval)
{
	u8 high, low;
	u16 data = 0;
	...
	if(ret) {
		dev_err(&st->spi->dev,
			"Writing start conversion register failed %d\n",
			ret);
		return ret;
	}

	ret = iio_adc_emu_spi_read(st, EMU_REG_CHAN_H(channel), &high);
	if(ret) {
		dev_err(&st->spi->dev,
			"Reading high conversion result register failed %d\n",
			ret);
		return ret;
	}

	ret = iio_adc_emu_spi_read(st, EMU_REG_CHAN_L(channel), &low);
	if(ret) {
		dev_err(&st->spi->dev,
			"Reading low conversion result register failed %d\n",
			ret);
		return ret;
	}
				...
				int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		if (!st->reg_select) {
			ret = iio_adc_emu_read_chan(st, chan->channel,
				(u16 *)val);
			if(ret) {
				dev_err(&st->spi->dev,
					"Reading from channel %d failed\n",
					chan->channel);
			}
			return IIO_VAL_INT;
		} else {
				 ...
				 int val2, long mask)
{
	...
	case IIO_CHAN_INFO_ENABLE:
		if(val) { //disable
			st->reg_select = 1;
			iio_adc_emu_spi_write(st, EMU_PWR_REG, EMU_PWR_DISABLE);
		} else {
			st->reg_select = 0;
			iio_adc_emu_spi_write(st, EMU_PWR_REG, EMU_PWR_ENABLE);
		}
		return 0;
	default:
		return -EINVAL;
...
static irqreturn_t iio_adc_emu_trigger_handler(int irq, void *p) {
	...
	if(ret) {
		dev_err(&st->spi->dev,
			"Writing conversion in buffer register failed %d\n",
			ret);
	...
	{
		ret = iio_adc_emu_spi_read(st, EMU_REG_CHAN_H(bit), &high);
		if(ret) {
			dev_err(&st->spi->dev,
				"Reading high conversion result register failed %d\n",
				ret);
		...
		ret = iio_adc_emu_spi_read(st, EMU_REG_CHAN_L(bit), &low);
		if(ret) {
			dev_err(&st->spi->dev,
				"Reading low conversion result register failed %d\n",
				ret);
...
MODULE_DESCRIPTION("IIO ADC EMU");
