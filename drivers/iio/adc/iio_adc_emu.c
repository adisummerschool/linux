...
static int iio_adc_emu_read_chan(struct iio_adc_emu_st *st, int channel, u16 *readval)
{
    ...
    iio_adc_emu_spi_write(st, EMU_REG_CONST, FIELD_PREP(EMU_CNVST_START,1));
...
static int iio_adc_emu_debugfs_reg_acces(struct iio_dev *indio_dev,
...
static irqreturn_t iio_adc_emu_trigger_handler(int irq, void *p)
{
    ...
    ret = iio_adc_emu_spi_write(st, EMU_REG_CONST, 
                                FIELD_PREP(EMU_CNVST_START,1));

      if(ret){
        dev_err(&st->spi->dev, "Writing high register failed in trigger: %d\n", ret);
...
static const struct iio_chan_spec iio_adc_emu_channels[] = {
 ...
 static const struct iio_info iio_adc_emu_info = {
      ...
      .debugfs_reg_access = &iio_adc_emu_debugfs_reg_acces
};
