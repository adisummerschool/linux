...
static int iio_adc_emu_spi_read(struct iio_adc_emu_st *st, u8 addr, u8 *data)
{
	...
	if (ret) {
		dev_info(&st->spi->dev,
			"SPI READ transfer failed with error code %d\n",
			ret);
		return ret;
	}
...
static int iio_adc_emu_spi_write(struct iio_adc_emu_st *st, u8 addr, u8 data)
{
	...
	tx = FIELD_PREP(EMU_RDWR_MSK, 0) |
	     FIELD_PREP(EMU_ADDR_MSK, addr) |
	     FIELD_PREP(EMU_DATA_MSK, data);

	put_unaligned_be16(tx, &payload);

	//dev_info(&st->spi->dev, "constructed package: %x\n", payload);
	//dev_info(&st->spi->dev, "original tx: %x\n", tx);

	return spi_sync_transfer(st->spi, &t, 1);
}

static int iio_adc_emu_read_chan(struct iio_adc_emu_st *st,
				      int channel,
				      u16 *readval)
{
	u8 high = 0;
	...
	ret = iio_adc_emu_spi_write(st,
			      	    EMU_REG_CNVST,
				    FIELD_PREP(EMU_CNVST_START, 1));

	if (ret) {
		dev_err(&st->spi->dev,
			"Conversion start failed with err code %d\n",
			ret);
		return ret;
	}
	...
	if (ret) {
		dev_err(&st->spi->dev,
			"Conversion read HIGH failed with err code %d\n",
			ret);
		return ret;
	}
	...
	if (ret) {
		dev_err(&st->spi->dev,
			"Conversion read LOW failed with err code %d\n",
			ret);
		return ret;
	}

	*readval = FIELD_PREP(EMU_HIGH_DATA_MSK, high) | low;
				...
				long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		if (!st->reg_select) {
			ret = iio_adc_emu_read_chan(st,
						    chan->channel,
						    (u16 *) val);
			if (ret) {
				dev_err(&st->spi->dev,
					"Read from channels failed\n");
				return ret;
			}
...
static irqreturn_t iio_adc_emu_trigger_handler(int irq, void *p)
{
	...
	if (ret) {
		dev_err(&st->spi->dev,
			"Writing conversion in buffer reg failed\n");
			...
			indio_dev->num_channels)
	{
		...
		if (ret) {
			dev_err(&st->spi->dev,
				"Reading HIGH register failed with err code %d\n",
				ret);
			iio_trigger_notify_done(indio_dev->trig);
			return IRQ_HANDLED;
		}
		...
		if (ret) {
			dev_err(&st->spi->dev,
				"Reading LOW register failed with err code %d\n",
				ret);
			iio_trigger_notify_done(indio_dev->trig);
			return IRQ_HANDLED;
		}
