...
static int iio_adc_emu_read_chan(struct iio_adc_emu_st *st, int channel, u16 *readval)
{
        ...
        ret = iio_adc_emu_spi_write(st, EMU_REG_CHAN_HIGH(channel), &high);
        if(ret) {
                dev_err(&st->spi->dev, "Reading high register failed: %d\n", ret);
                return ret;
        }

        ret = iio_adc_emu_spi_write(st, EMU_REG_CHAN_LOW(channel), &low);
        if(ret) {
                ...
                case IIO_CHAN_INFO_RAW:
                        if(!st->reg_select){
                                if(chan->channel) {
                                        ...
                                        dev_info(&indio_dev->dev, "Trying to wite to channel 0");
                                        // st->chan_val[0] = val;
                                }
                                return 0;
                        }
