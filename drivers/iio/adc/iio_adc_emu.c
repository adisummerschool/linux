...
static int iio_adc_emu_spi_read(struct iio_adc_emu_st *st,
				u8 addr, u8 *data)
{
	u8 tx = 0;
	u8 rx = 0;
	...
	if (ret) {
		dev_err(&st->spi->dev, "SPI read transfer failed\n");
		return ret;
	}
...
static int iio_adc_emu_spi_write(struct iio_adc_emu_st *st,
				 u8 addr, u8 data)
{
	u16 tx = {0};
	u16 packed = 0;
...
static const struct iio_info iio_adc_emu_info = {
 ...
 static int iio_adc_emu_probe(struct spi_device *spi) {
	...
	st->reg_select = 1; // logica inversa
	st->spi = spi;
