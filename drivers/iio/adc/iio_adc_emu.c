...
#define EMU_ADDRESS_MSK	GENMASK(14, 8)
...
static int iio_adc_emu_spi_read(struct iio_adc_emu_st* st, u8 address, u8* data) {
	u8 tx = 0;
	u8 rx = 0;
	int ret = 0;
	...
	tx = FIELD_PREP(EMU_RDWR_MSK, 1) | address;
...
static int iio_adc_emu_spi_write(struct iio_adc_emu_st* st, u8 address, u8 data) {
	u16 tx = 0;
	u16 package = 0;
	struct spi_transfer t[] = {
		{
	...
	tx = FIELD_PREP(EMU_RDWR_MSK, 0) | FIELD_PREP(EMU_ADDRESS_MSK, address)
...
static int iio_adc_emu_read_chan(struct iio_adc_emu_st *st, int channel, u16 *readval) {
	...
	if(ret) {
		dev_err(&st->spi->dev, "Reading high register failed with error: %d\n", ret);
		return ret;
	}
	ret = iio_adc_emu_spi_read(st, EMU_REG_CHAN_LOW(channel), &low);
	if(ret) {
		dev_err(&st->spi->dev, "Reading low register failed with error: %d\n", ret);
		return ret;
	}
...
static irqreturn_t iio_adc_emu_trigger_handler(int irq, void *p) {
	...
	ret = iio_adc_emu_spi_write(st, EMU_REG_CNVST, FIELD_PREP(EMU_CNVST_START, 1));
	if(ret) {
		dev_err(&st->spi->dev, "Writing conversion in buffer failed with error: %d\n", ret);
	...
	for_each_set_bit(bit, indio_dev->active_scan_mask, indio_dev->num_channels) {
		...
		if(ret) {
			dev_err(&st->spi->dev, "Reading high register failed in trigger with error: %d\n", ret);
			iio_trigger_notify_done(indio_dev->trig);
			return IRQ_HANDLED;
		}
		ret = iio_adc_emu_spi_read(st, EMU_REG_CHAN_LOW(bit), &low);
		if(ret) {
			dev_err(&st->spi->dev, "Reading low register failed in trigger with error: %d\n", ret);
			iio_trigger_notify_done(indio_dev->trig);
			return IRQ_HANDLED;
		}
