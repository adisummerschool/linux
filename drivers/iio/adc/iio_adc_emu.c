 ...
 */
 ...
 static int iio_adc_emu_read_chan(struct iio_adc_emu_st *st, int channel, u16 *readval)
 {
   ...
   u16 data = 0;
   int ret;
   ret = iio_adc_emu_spi_write(st, EMU_REG_CNVST, FIELD_PREP(EMU_CNVS_START, 1));
   if (ret) {
 ...
 static irqreturn_t iio_adc_emu_trigger_handler(int irq, void *p)
 {
   ...
   u16 buf[2];
     ...
     ret = iio_adc_emu_spi_write(st, EMU_REG_CNVST, FIELD_PREP(EMU_CNVS_START, 1));
   if (ret) {
      dev_err(&st->spi->dev, "Writing conversion in buffer failed: %d\n", ret);
 ...
 static int iio_adc_emu_probe(struct spi_device *spi)
 {
    ...
    if(ret)
    {
      dev_err(&spi->dev, "Failed to craete buffer");
      return ret;
    }
