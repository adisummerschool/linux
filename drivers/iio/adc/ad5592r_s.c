...
#define AD5592R_MSB_MSK                 BIT(15)
#define AD5592R_ADDR_MSK                GENMASK(14, 11)
#define AD5592R_DATA_MSK                GENMASK(10, 0)

#define AD5592R_RDB_REG_ADDR            0x7
#define AD5592R_RDB_ENABLE              BIT(6)
#define AD5592R_RDB_REG_SELECT          GENMASK(5, 2)
...
#define AD5592R_REG_CONFIG_DATA         GENMASK(5, 0)

#define AD5592R_CNV_DATA_MSK            GENMASK(11, 0)
...
static int ad5592r_s_spi_write(struct ad5592r_s_st *st, u8 addr, u16 data)
{
        ...
        tx = FIELD_PREP(AD5592R_MSB_MSK, 0) |
...
static int ad5592r_s_spi_read(struct ad5592r_s_st *st, u8 addr, u16 *data)
{
        ...
        reg_rdb_data = FIELD_PREP(AD5592R_RDB_ENABLE, 1) |
                FIELD_PREP(AD5592R_RDB_REG_SELECT, addr);

        ret = ad5592r_s_spi_write(st, AD5592R_RDB_REG_ADDR, reg_rdb_data);
        if(ret) {
...
static int ad5592r_s_read_chan(struct ad5592r_s_st *st, int channel, u16 *readval)
{
        ...
        ret = ad5592r_s_spi_write(st, AD5592R_REG_SEQ_ADDR, 1 << channel);
        if(ret) {
                dev_err(&st->spi->dev, "Writing ADC seq register failed: %d\n", ret);
                return ret;
        }

        ret = spi_sync_transfer(st->spi, &t, 1);
        if(ret) {
                dev_err(&st->spi->dev, "Starting ADC conversion failed");
                return ret;
        }

        ret = spi_sync_transfer(st->spi, &t, 1);
        if(ret) {
                dev_err(&st->spi->dev, "ADC conversion result failed");
                return ret;
        }

        data = get_unaligned_be16(&rx);

        *readval = FIELD_GET(AD5592R_CNV_DATA_MSK, data);
...
static irqreturn_t ad5592r_s_trigger_handler(int irq, void *p)
{
        ...
        for_each_set_bit(bit, indio_dev->active_scan_mask, indio_dev->num_channels) {
                ret = ad5592r_s_spi_write(st, AD5592R_REG_SEQ_ADDR, 1 << bit);
                if (ret) {
                        dev_err(&st->spi->dev, "Writing ADC seq register failed in trigger: %d\n", ret);
                ...
                ret = spi_sync_transfer(st->spi, &t, 1);
                if (ret) {
                        dev_err(&st->spi->dev, "Starting ADC conversion failed in trigger");
                        iio_trigger_notify_done(indio_dev->trig);
                        return IRQ_HANDLED;
                }
                ...
                ret = spi_sync_transfer(st->spi, &t, 1);
                if (ret) {
                        dev_err(&st->spi->dev, "ADC conversion result failed in trigger");
                        iio_trigger_notify_done(indio_dev->trig);
                        return IRQ_HANDLED;
                }

                data = get_unaligned_be16(&rx);
                buff[i++] = FIELD_GET(AD5592R_CNV_DATA_MSK, data);
        }
...
static int ad5592r_s_probe(struct spi_device *spi)
{
	...
	indio_dev->num_channels = ARRAY_SIZE(ad5592r_s_channels);
        ...
        ad5592r_s_spi_write(st, AD5592R_REG_PD_ADDR, FIELD_PREP(AD5592R_REG_EN_IREF, 1));
        ad5592r_s_spi_write(st, AD5592R_REG_CONFIG_ADDR, AD5592R_REG_CONFIG_DATA);
